...
def search_i(text,chat,bot):
    ...
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':
            ...
            bot.sendMessage(chat['object_guid'], 'ناتایج کامل به زودی به پیوی شوما ایرسال میشود🗿🤝', chat['last_message']['message_id'])                           
    ...
    try:
        ...
        if user_info['data']['exist'] == True:
            ...
            if user_info['data']['type'] == 'User':
                ...
                bot.sendMessage(chat['object_guid'], 'نام 🗿:\n  ' + user_info['data']['user']['first_name'] + ' ' + user_info['data']['user']['last_name'] + '\n\nبیوگرافی 🐸:\n   ' + user_info['data']['user']['bio'] + '\n\nguid 😟:\n  ' + user_info['data']['user']['user_guid'] , chat['last_message']['message_id'])

                print('sended response')

            else:

                bot.sendMessage(chat['object_guid'], 'کاناله داپش🗿' , chat['last_message']['message_id'])

                print('sended response')

        else:

            bot.sendMessage(chat['object_guid'], 'کاربری با این آیدی وجود نداره داپش🗿✌🏿' , chat['last_message']['message_id'])
...
def search(text,chat,bot):
    ...
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':                               
            ...
            bot.sendMessage(chat['object_guid'], 'ناتایج رو فیرستادم پیویت داپش😟🗿', chat['last_message']['message_id'])

            bot.sendMessage(chat['last_message']['author_object_guid'], 'ناتایج یافت شده برای (' + search + ') : \n\n'+text)
        ...
        bot.sendMessage(chat['object_guid'], 'داوش این محدود شده یا داره تعمیر میشه✌🏿🐸' , chat['last_message']['message_id'])
...
def get_font(text,chat,bot):
    ...
    try:
        ...
        if hasInsult(name_user)[0] == False and chat['abs_object']['type'] == 'Group':
            ...
            bot.sendMessage(chat['object_guid'], 'ناتایج کامل رو فرستادن پیویت داپش🐸🤝', chat['last_message']['message_id'])

            bot.sendMessage(chat['last_message']['author_object_guid'], 'ناتایج یافت شده برای (' + name_user + ') : \n\n'+text)                                        
    ...
    try:
        ...
        if hasInsult(mozoa)[0] == False and chat['abs_object']['type'] == 'Group' and page > 0:
            ...
            if not 'codebazan.ir' in text_t:
                ...
                bot.sendMessage(chat['last_message']['author_object_guid'], 'ناتایج یافت شده برای (' + mozoa + ') : \n\n'+text)
...
def get_font_fa(text,chat,bot):
    ...
    try:
        ...
        if hasInsult(site)[0] == False and chat['abs_object']['type'] == 'Group':
            ...
            bot.sendMessage(chat['last_message']['author_object_guid'], 'ناتایج یافت شده برای (' + site + ') : \n\n'+text)                                        
...
def get_added(text,chat,bot):    
    ...
    try:
        ...
        send_text = 'سلاپ داوش/خواهر به گروه ' + group + ' خوش اومدی 🐸😟 \n لطفا قاوانین را رعایت کون تا حذف نشی🗿🤝'
                        ...
                        if not m_id in list_message_seened:
                            ...
                            if text == '/start':
                                ...
                                try:
                                    ...
                                    bot.sendMessage(chat['object_guid'], 'سلاپ🗿 \n به گودرتمند ترین بات روبیکا خوش اومدی داپش / خواهر ❤😟\n\n لطفا برای آشنایی با دستورات ربات \n/help \nرا ایرسال کون😟🤝',chat['last_message']['message_id'])
                                ...
                                try:
                                    ...
                                    bot.sendMessage(chat['object_guid'], "دارم لینک رو آماده موکونم داپش یکم صبر کون 🐸🤝",chat['last_message']['message_id'])
                                    ...
                                    try:
                                        ...
                                        bot.sendMessage(chat['object_guid'], 'لینک نیم بهات آماده شود داپش🐸 ✅ \n لینک : \n' + nim_baha ,chat['last_message']['message_id'])
